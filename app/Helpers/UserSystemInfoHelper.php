...
class UserSystemInfoHelper
{
	...
	public static function createDevice($uuid = null): Device
	{
		$ip                       = self::get_ip();
		$device                   = new Device();
		$device->ip               = $ip;
		$device->uuid             = $uuid ?: Str::uuid();
		$device->user_agent       = self::get_user_agent();
		$device->device           = self::get_device();
		$device->operating_system = self::get_os();
		$country                  = (new self)->get_country_from_ip($ip);
		$device->country_code     = $country['country_code'];
		$device->country_name     = $country['country'];
