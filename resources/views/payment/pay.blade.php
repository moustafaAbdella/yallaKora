...
<head>
    ...
    @php
        ...
        if ($mode == 'sandbox') {
            $url = config('paypal.sandbox.client_id');
        } else {
            $url = config('paypal.live.client_id');
        }
    @endphp
    <script src="https://www.paypal.com/sdk/js?client-id={{ $url }}&currency=USD"></script>
...
<div class="iphone">
    <header class="header">
        <h1>إشتراك بريميوم</h1>
    </header>

    {{-- <form action="{{route('payment-mobile')}}?user_id={{ $user_id }}&plan_id={{ $plan->id }}" class="form" method="POST"> --}}
    <div>
        {{-- <h2>خطة</h2> --}}
    ...
    <fieldset>
        <legend style="padding-top: 10px;">طريقة الدفع</legend>
        ...
        <input checked id="visa" name="type"  value="visa" type="radio" />
      </div> --}}
            ...
            <use xlink:href="#icon-paypal" />
                ...
                {{-- <input checked id="paypal" name="type" value="paypal" type="radio" /> --}}
            </div>
...
@php
    $payment_history = \App\Models\Payment::create([
        'user_id' => $user_id,
...
@endphp
<script>
    // alert("⚠️ لا تغلق هذه الصفحة لحين انتهاء عملية الإشتراك  \n 🎁 سيصلك كود التفعيل هنا");
    paypal.Buttons({
        createOrder: function(data, actions) {
            return fetch('/checkout/api/paypal/order/create?user_id={{$user_id}}&plan_id={{$plan->id}}', {
                method: 'post',
                headers: {
        ...
        onApprove: function(data, actions) {
            ...
            window.location =
                `/checkout/api/paypal/order/execute?type=paypal&status=${data.status}&orderId=${data.orderID}&PayerID=${data.payerID}`;
            // });
            // return fetch('/paypal/execute-order', {
            //     method: 'post',
            //     headers: {
            //         'content-type': 'application/json'
            //     },
